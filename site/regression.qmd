...
## Model Free Interpretation of Regression
...
- **caution**, can have $\Er[y|x]$ strictly increasing in
  $x_j$, but $\beta_{0,j} < 0$.

## Caution

- Regression being a linear approximation to $\Er[Y|X]$ does not mean
$\beta$ necessarily has the sign you want
...
print(f"mean( E[y|x1=1,x2] - E[y|x1=0,x2] ) = {mean_dEydx2:.3}")
```
...
- Regression estimates $\beta_0 \in \argmin_{\beta} \Er[(\Er[y|x] - x'\beta)^2]$
  ...
  - If $\Er[y|x]$ is linear, then $\frac{\partial}{\partial x_1}\Er[y|x] = \beta_{0,1}$ is the
  change in $y$ when changing $x_1$ holding the rest of $x$ constant
