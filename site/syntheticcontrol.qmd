...
$$
\mathbm{X}_1 = \left(Y_{11}, \cdots Y_{1T_0}, z_{11}, \cdots, z_1K \right)^T
$$
- Matrix of same variabels for untreated
$$
\mathbm{X}_0 = \begin{pmatrix}
...
$$
\begin{align*}
\hat{W} = & \textrm{arg}\min_{W \in \R^J} \Vert \mathbm{X}_1 - \mathbm{X}_0 W \Vert_V \\
...
- As $T_0$ increases or variance of $\epsilon_{jt}$ decreases, bias of $\hat{\tau}_{it}$ decreases
  - Needs $\mathbm{X}_1 \approx \mathbm{X}_0 W$

## Choices

::: {.incremental}
- Variables in $\mathbm{X}$ to match
   - Fewer make eaiser to have $\mathbm{X}_1 \approx \mathbm{X}_0 W$
   - But fewer make $W$ depend more on $\epsilon$

- Set of untreated units to consider
   - More units makes eaiser to have $\mathbm{X}_1 \approx \mathbm{X}_0 W$
...
$$
  ...
  - $\hat{w}_j$ depends on pre-treatment variables $\mathbm{X}_1, \mathbm{X}_0$, $J+1 \times (T_0 + K)$
  - $J$ values of $Y_{jt}$
...
\begin{align*}
\hat{\tau}_{1t} = & Y_{1t} -
\underbrace{\hat{Y}_{1t}(0)}_{\text{prediction given $Y_{jt}, \mathbm{X}_0$}}
\\
...
$$
P\left[ P\left(\tau_{1t} \in \mathcal{I}_{1-\alpha} | \mathbm{X}_0, \{y_{jt}\}_{j=1}^J\right) > 1-\alpha-\epsilon(T_0) \right] > 1 - \pi(T_0)
$$
