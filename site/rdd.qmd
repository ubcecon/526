...
- Many papers just show binned means, but better to show uncertainty / variability in data too
  - Two good options in `rdrobust` package:
     1. `binselect='es'` or `binselect='qs'` and plot confidence intervals
     2. `binselect='esmv'` or `binselect='qsmv'`
...
```{python}
rdpx = rdrobust.rdplot(df.x,df.r,c=0,hide=False, binselect='es',x_label="R",y_label="x",ci=95) #, #p=1,
                       #kernel="triangular", h=rdx.bws.loc['h'],
...
lrs = lrs_all.loc[lrs.eligible_saber11==1]
lrs.columns
```
